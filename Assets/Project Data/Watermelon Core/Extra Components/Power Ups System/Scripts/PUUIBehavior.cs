...
namespace Watermelon
{
    public class PUUIBehavior : MonoBehaviour
    {
        ...
        {
            if (GameGlobal.Instance.GetModule<RoleModule>().IsHavePuItem(settings.Type))
            {
                if (!behavior.IsBusy)
                {
        ...
        public void Redraw()
        {
            int amount = GameGlobal.Instance.GetModule<RoleModule>().GetPUAmount(this.settings.Type);
            if (amount > 0)
            {
